...
{
    [TestClass]
    public class ThreeGamesTest : BaseQueryTest
    {
        ...
        [TestMethod]
        public void GetPlayerGames_ThreeGames()
        {
            var query = new GetPlayerGamesQuery() { PlayerName = _player3 };
            RunTest<IEnumerable<GetPlayerGamesDto>>(query);
        }

        public override IEnumerable<IDataTransferObject> ExpectedDtos()
        {
            yield return new GetPlayerGamesDto() { GameId = _gameId1, GameDate = _gameDate1, PlayerName = _player3, Placing = 3, Winnings = 0, PayIn = 20 };
            yield return new GetPlayerGamesDto() { GameId = _gameId2, GameDate = _gameDate2, PlayerName = _player3, Placing = 3, Winnings = 0, PayIn = 20 };
            yield return new GetPlayerGamesDto() { GameId = _gameId3, GameDate = _gameDate3, PlayerName = _player3, Placing = 1, Winnings = 60, PayIn = 40 };
        }
    }
}
