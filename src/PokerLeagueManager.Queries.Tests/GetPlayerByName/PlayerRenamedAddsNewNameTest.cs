...
{
    [TestClass]
    public class PlayerRenamedAddsNewNameTest : BaseQueryTest
    {
        ...
        {
            RunTest(x => x.GetPlayerByName(_newPlayerName));
        }

        public override IDataTransferObject ExpectedDto()
        {
            return new GetPlayerByNameDto() { DtoId = AnyGuid(), PlayerName = _newPlayerName, GameCount = 1 };
        }
    }
}
