...
{
    [TestClass]
    public class CaseInsensitiveTest : BaseQueryTest
    {
        ...
        [TestMethod]
        public void GetPlayerByName_CaseInsensitive()
        {
            var query = new GetPlayerByNameQuery() { PlayerName = "  dylan " };
            RunTest(query);
        }

        public override IDataTransferObject ExpectedDto()
        {
            return new GetPlayerByNameDto() { DtoId = AnyGuid(), PlayerName = _player1, GameCount = 1 };
        }
    }
}
